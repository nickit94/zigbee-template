...
#define ZB_ZCL_GET_ATTR_FLOAT(p_value) (*(float*)p_value)
 ...
 */
static zb_ret_t check_value_analog_input(zb_uint16_t attr_id, zb_uint8_t endpoint, zb_uint8_t * p_value)
{
    ...
    switch(attr_id)
    {
        ...
        {   
            ...
            if (val != ZB_TRUE && 
                val != ZB_FALSE) 
            {
                NRF_LOG_INFO("ERR: Check value ZB_ZCL_ATTR_ANALOG_VALUE_OUT_OF_SERVICE_ID - fail");
                break;
            }
        ...
        {
            float val = ZB_ZCL_GET_ATTR_FLOAT(p_value);
            NRF_LOG_DEBUG("Validating value %ld of Analog Input %d", (int32_t)val, attr_id);
        ...
        {    
            ...
            if (val > 0x0F) 
            {
                NRF_LOG_INFO("ERR: Check value ZB_ZCL_ATTR_ANALOG_VALUE_STATUS_FLAGS_ID - fail");
                break;
            }
 ...
 */
static zb_void_t zb_zcl_analog_input_write_attr_hook(zb_uint8_t endpoint, zb_uint16_t attr_id, zb_uint8_t * new_value)
{
    UNUSED_PARAMETER(new_value);
    NRF_LOG_DEBUG("Writing attribute %d of Analogstate Input Cluster on endpoint %d", attr_id, endpoint);

    if (attr_id == ZB_ZCL_ATTR_ANALOG_INPUT_PRESENT_VALUE_ID)
    {
	      /* Implement your own write attributes hook if needed. */
    }
}

/**@brief Function which initialises the server side of Analogstate Input Cluster. */
zb_void_t zb_zcl_analog_input_init_server(void) 
{
    zb_ret_t ret = zb_zcl_add_cluster_handlers(ZB_ZCL_CLUSTER_ID_ANALOG_INPUT,
                                               ...
                                               check_value_analog_input,
                                               zb_zcl_analog_input_write_attr_hook,
                                               (zb_zcl_cluster_handler_t)NULL);
    ASSERT(ret == RET_OK);
}

/**@brief Function which initialises the client side of Analogstate Input Cluster. */
