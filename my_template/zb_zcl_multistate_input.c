 ...
 */
static zb_ret_t check_value_multi_input(zb_uint16_t attr_id, zb_uint8_t endpoint, zb_uint8_t * p_value)
{
    ...
    switch(attr_id)
    {
        ...
        case ZB_ZCL_ATTR_MULTI_INPUT_PRESENT_VALUE_ID:
        {
            zb_uint16_t val = ZB_ZCL_ATTR_GET16(p_value);
            NRF_LOG_DEBUG("Validating value %d of Multistate Input %d", val, attr_id);
 ...
 */
static zb_void_t zb_zcl_multi_input_write_attr_hook(zb_uint8_t endpoint, zb_uint16_t attr_id, zb_uint8_t * new_value)
{
    UNUSED_PARAMETER(new_value);
    NRF_LOG_DEBUG("Writing attribute %d of Multistate Input Cluster on endpoint %d", attr_id, endpoint);

    if (attr_id == ZB_ZCL_ATTR_MULTI_INPUT_PRESENT_VALUE_ID)
    {
	      /* Implement your own write attributes hook if needed. */
    }
}

/**@brief Function which initialises the server side of Multistate Input Cluster. */
zb_void_t zb_zcl_multi_input_init_server(void) 
{
    zb_ret_t ret = zb_zcl_add_cluster_handlers(ZB_ZCL_CLUSTER_ID_MULTI_INPUT,
                                               ZB_ZCL_CLUSTER_SERVER_ROLE,
                                               check_value_multi_input,
                                               zb_zcl_multi_input_write_attr_hook,
                                               (zb_zcl_cluster_handler_t)NULL);
    ASSERT(ret == RET_OK);
}

/**@brief Function which initialises the client side of Multistate Input Cluster. */
